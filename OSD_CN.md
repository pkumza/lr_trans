...
尽管我们专注于完整的主板而不是SoC，但是文本中的大部分内容都会推广到使用相同SoC的其他主板。例如，与Galileo一起使用的上下文切换代码也可在其他x86平台上运行，BeagleBone Black上使用的上下文切换代码也可在其他ARM平台上运行。

3.19 观点
============
处理器或I/O设备的硬件规格包含了太多细节，以至于详细地研究它们似乎是不可能的。性欲的是，处理器之间的区别很多都在表面——基本概念可以适用于绝大多数的硬件平台。因此，在学习硬件时，重点应该放在整体架构和设计原则，而不是具体的细节。
